...
export default function handler(req, res) {
  ...
  const START_AT = (PAGE - 1) * LIMIT;
  const END_AT = PAGE * LIMIT;

  const list = TODOS.slice(START_AT, END_AT);
  const total = TODOS.length;
  const totalPage = Math.round(total / LIMIT);

  res.status(200).json({ todos: list, page: PAGE, limit: LIMIT, total: TODOS.length, totalPage });
}
